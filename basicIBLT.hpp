...
class IBLT_bucket {
  public:
	...
	void remove(IBLT_bucket<key_type> b) {
		key_sum ^= b.key_sum;
		hash_sum ^= b.hash_sum;
		count += (-1)*b.count;
	}

	void XOR(IBLT_bucket<key_type> counterparty_bucket) {
    ...
    bool operator<( const IBLT_bucket& other ) const
    {
...
class IBLT {
  public:
	...
	IBLT(int bucket_count, int num_hashfns): 
		...
		key_hasher.set_seed_ind(0);
		for(int i = 0; i < num_hashfns; ++i) {
			subIBLTs[i].resize(buckets_per_subIBLT);
			sub_hashers[i].set_seed_ind(i+1); //separate seeds enough
		}
	}

	void XOR(IBLT<key_type>& counterparty) {
	...
	bool peel(std::set<key_type>& peeled_keys ) {
